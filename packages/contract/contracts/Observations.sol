...
contract Observations {
    ...
    /// @param x The x coordinate on the artifact (0 = not located).
    /// @param y The y coordinate on the artifact (0 = not located).
    ...
    function observe(
        ...
        emit Observation(collection, tokenId, msg.sender, id, parent, update,
                         note, x, y, viewType, time,
                         msg.value, tipRecipient);
    }
    ...
    function claimTips(address tipRecipient) external {
        ...
        if (!authorized && msg.sender == ClaimableTips.UNCLAIMED_TIPS_RECIPIENT) {
            if (block.timestamp - tips[tipRecipient].unclaimedSince <= 365 days)
              revert TipsNotYetClaimable();
            authorized = true;
        }
